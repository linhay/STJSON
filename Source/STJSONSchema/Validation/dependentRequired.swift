func dependentRequired(context: Context, dependentRequired: Any, instance: Any, schema: [String: Any]) throws -> AnySequence<ValidationError> {
  ...
  return try AnySequence(dependentRequired.compactMap({ (key, value) -> AnySequence<ValidationError> in
    if instance.keys.contains(key) {
      return try required(context: context, required: value, instance: instance, schema: schema)
    }
